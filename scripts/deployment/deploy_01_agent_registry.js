...
async function main() {
    ...
    let agentRegistry = await AgentRegistry.connect(EOA).deploy(agentRegistryName, agentRegistrySymbol, baseURI);
    if (gasPriceInGwei === "0") {
        ///
    } else {
        const gasPrice = ethers.utils.parseUnits(gasPriceInGwei, "gwei");
        agentRegistry = await AgentRegistry.connect(EOA).deploy(agentRegistryName, agentRegistrySymbol, baseURI, { gasPrice });
    }
    ...
    if (parsedData.contractVerification) {
        ...
        execSync("npx hardhat verify --constructor-args scripts/deployment/verify_01_agent_registry.js --contract contracts/AgentRegistryX.sol:AgentRegistryX --network " + providerName + " " + agentRegistry.address, { encoding: "utf-8" });
    }
}
